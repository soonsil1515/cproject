...
int main(void)
{
    ...
    end_clock = time(NULL); 
    printf("시간 측정이 끝났습니다...\n");
    printf("e-s %f", (float)(end_clock - start_clock));
    //save_rank(name, ex_time, map_num);
    printf("\nMAP1을 성공하셨습니다!\n");
    printf("--------------------------\n\n");
...
void display(void){
    ...
    printf("* r(reply) : 현재 맵을 처음부터 다시 시작(게임시간은 계속 유지\n\n");
...
void save_rank(char name[], float ex_time,int map_num)
{
    char temp[10] = {0};
    char rank_name[5][5][20] = {0};
    float rank_time[5][5] = {0};
    FILE *rank;
    ...
    for(int i = 0 ; i < 5 ; i++){
        for(int j = 4 ; j >= 0 ; j--){
            if((rank_time[i][j] > ex_time) && j){
                rank_time[i][j] = rank_time[i][j-1];
                for(int k = 0 ; k < 20 ; i++){
                    rank_name[i][j][k] = rank_name[i][j-1][k];
                }
            }
            else {
                if(j == 4) break;
                else if((j == 0) && (ex_time < rank_time[i][j])) {
                    rank_time[i][0] = ex_time;
                    for(int k = 0 ; k < 20; k++)
                        rank_name[i][0][k] = name[k];
                }
                else{ 
                    rank_time[i][j+1] = ex_time;
                    for(int k = 0 ; k < 20 ; k++)
                        rank_name[i][j+1][k] = name[k];
                }
            }
        }
    }
